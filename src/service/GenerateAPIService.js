...
class Service extends BaseService {
   ...
   * @param {*} parameters
   */
  async getAnalysisOutPut({ url, body }) {
    const output = {};
    const width = 1024;
    const height = 1024;
    try {
      ...
      applicationPage.setDefaultNavigationTimeout(2000000);
      await applicationPage.setViewport({ width: width, height: height });
      await applicationPage.setUserAgent("UA-TEST");
      // 导航到指定地址
      await applicationPage.goto(url,{waitUntil: 'domcontentloaded'});
      // 延时等待3秒
  ...
  async analysisContent(analysis, output) {
    // 如果有前置操作则触发一次点击操作
    if (analysis.before) {
      await this.clickElement(analysis.before.selector, applicationPage);
    }
    // 如果有输出则输出内容
    if (analysis.output) {
      for (let i = 0; i < analysis.output.length; i++) {
        ...
        try {
          // 任何输出都是往对象中添加属性
          value = await applicationPage.evaluate((itme) => {
    ...
    if (analysis.after) {
      await this.clickElement(analysis.after.selector, applicationPage);
    }
    if (analysis.children) {
      ...
      let newNodes = [];
      let nowNode = applicationPage
      if (analysis.children.root) {
        nowNode = await applicationPage.$(analysis.selector);
      } 
