...
const DevtoolDatabaseDAO = require('../dao/DevtoolDatabaseDAO.js')
const DevtoolTemplateDAO = require('../dao/DevtoolTemplateDAO.js')
const DevtoolTemplateFileDAO = require('../dao/DevtoolTemplateFileDAO.js')
const DevtoolGeneratorConfigDAO = require('../dao/DevtoolGeneratorConfigDAO.js')

const DevtoolAnalysisDAO = require('../dao/DevtoolAnalysisDAO.js')

const GenerateAPIService = require('../service/GenerateAPIService.js')
...
class Service extends BaseService {
    ...
    */
	async getTemplateInfo(templateName) {
		// 获取模板信息
		const templateInfo = await DevtoolTemplateDAO.query({
			name: templateName
		})
		this.notNull(templateInfo, '没有相关的模板信息')
		// 获取模板文件信息
		const templateFileInfo = await DevtoolTemplateFileDAO.query({
    ...
    */
	async getTemplateData(parameters) {
		// 获取必要数据
		// 获取表格信息
		let tableInfo = await DevtoolDatabaseDAO.getTableInfo({
		...
		// 获取配置信息
		let generatorConfigInfo = await DevtoolGeneratorConfigDAO.query({
    ...
    */
	async buildByAnalysis({templateName, analysisId}) {
		...
		// 获取分析配置
		const analysisInfo = await DevtoolAnalysisDAO.query({
			id: analysisId
		})
		this.notNull(analysisInfo, '没有相关的分析配置信息')
		// 分析目标网页
		const analysisData = await GenerateAPIService.getAnalysisOutPut({ 'url': analysisInfo[0].url, 'body': analysisInfo[0].body })
		// 渲染
		const res = {}
		try {
			for (const i in templateFileInfo) {
