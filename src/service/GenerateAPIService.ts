...
export default class Service extends Basicservice {
  ...
  /**
   * 延迟等待的时间越长获取数据越稳定，在设备性能不变的情况下。
   * 设备性能越低，延迟时间越长，在获取数据稳定的情况下。
   * 原因是还没来得及将dom节点挂载（节点游离在文档树之外），就进行操作，导致报错。
   */
  ...
  async analysisContent(analysis: any, output: any) {
    ...
    if (analysis.children) {
      ...
      console.log("document.querySelectorAll('" + analysis.children.selector + "')", '节点元素个数', newNodes?.length)
      // 循环所有节点
      for (let i = 0; i < newNodes.length; i++) {
        ...
        try {
          ...
          await this.analysisContent(analysis.children, output[analysis.children.name][i])
        } catch (error) {
