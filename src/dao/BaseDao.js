...
class BaseDao {
	...
	add(val) {
		...
		if (_sql != '') {
			return this.MySqlConnect.query(_sql, _valueList)
		} else {
	...
	remove(val) {
		...
		try {
			...
			for (const key in val) {
				if (key == 'id') {
					const isId = Array.isArray(val[key])
					const ids = isId ? val[key] : [val[key]]
					let _id_sql = ' IN ( '
	...
	query(val) {
		...
		const pageNumber = (val.page_number || val.pageNumber) ? val.page_number - 1 : 0
		const pageSize = (val.page_size || val.pageSize) ? val.page_size : 1
     ...
     */
	queryTotal() {
		var _sql = 'SELECT FOUND_ROWS() as total'
		if (_sql != '') {
